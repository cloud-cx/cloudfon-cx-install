...
services:
  ...
  cx-chromadb:
    ...
    container_name: cx-chromadb
    ports:
      - "58000:8000"
    volumes:
      - ./chromadb_data/data:/chroma/chroma
      - /etc/timezone:/etc/timezone:ro
      - /etc/localtime:/etc/localtime:ro
    restart: always
    networks:
...
create() {
	...
	options=$(getopt -o i:t:m:r:w:u:b:s:d:ph --long image:,install_type:,mariadb_password:,redis_password:,middleware_ip:,mariadb_user:,mariadb_port:,redis_port:,mariadb_database:,chromadb_port: -- "$@")
	eval set -- "$options"
	...
	while true; do
	  case $1 in 
		...
		-c | --chromadb_port) shift; chromadb_port=$1 ; shift ;;
	...
	if [ "$help" = true ]; then
		...
		echo "	-s,--chromadb_port				install_type=api) remote chromadb port;install_type=mid) local chromadb External mapping port ; install_type=all)not effect; default 58000";
		return
	fi
	...
	if [ "$print" = true ]; then
		echo "image: $image; ai_image: $ai_agent_image;install_type: $install_type; mariadb_password: $mariadb_password;redis_password: $redis_password;middleware_ip: $middleware_ip;mariadb_user: $mariadb_user;chromadb_port:$chromadb_port";
	fi
	...
	case $install_type in
		api)
			...
			export_env $middleware_ip $mariadb_user $mariadb_password $mariadb_port $mariadb_database $middleware_ip $redis_password $redis_port $middleware_ip $chromadb_port $middleware_ip $jvm_mem
		...
		mid)
			...
			export_env $middleware_ip $mariadb_user $mariadb_password $mariadb_port $mariadb_database $middleware_ip $redis_password $redis_port $middleware_ip $chromadb_port $middleware_ip
		...
		*)
			...
			redis_url='cx-redis'
			chromadb_url='cx-chromadb'
			mariadb_port=3306
			redis_port=6379
			chromadb_port=8000
			middleware_ip='127.0.0.1'

      echo "$mariadb_url $mariadb_user $mariadb_password $mariadb_port $mariadb_database $redis_url $redis_password $redis_port $chromadb_url $chromadb_port $middleware_ip  $jvm_mem"

			export_env $mariadb_url $mariadb_user $mariadb_password $mariadb_port $mariadb_database $redis_url $redis_password $redis_port $chromadb_url $chromadb_port $middleware_ip  $jvm_mem
			export_redis_conf $redis_password
			export_nginx_conf
			;;
		esac
