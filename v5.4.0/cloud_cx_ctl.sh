...
create() {
	...
	options=$(getopt -o i:t:m:r:w:u:b:s:d:c:e:f:ph --long image:,install_type:,mariadb_password:,redis_password:,middleware_ip:,mariadb_user:,mariadb_port:,redis_port:,mariadb_database:,chromadb_port:,es_port:,es_password: -- "$@")
	eval set -- "$options"
	...
	if [ "$help" = true ]; then
		...
		echo "	-c,--chromadb_port				install_type=api) remote chromadb port;install_type=mid/all) local chromadb External mapping port ; default 58000";
	...
	if [ "$print" = true ]; then
		echo "image: $image; ai_image: $ai_agent_image;install_type: $install_type; mariadb_password: $mariadb_password;redis_password: $redis_password;middleware_ip: $middleware_ip;mariadb_user: $mariadb_user;chromadb_port:$chromadb_port;es_port:$es_port;es_password:$es_password";
	fi
	...
	case $install_type in
		api)
			...
			export_env $middleware_ip $mariadb_user $mariadb_password $mariadb_port $mariadb_database $middleware_ip $redis_password $redis_port $middleware_ip $chromadb_port $middleware_ip $jvm_mem $es_port $es_password $middleware_ip
